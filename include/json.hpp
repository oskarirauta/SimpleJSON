 ...
 *<oskari.rauta@gmail.com > made it compatible again with original SimpleJSON
 *by removing giri namespace
 *
...
namespace json {
	...
	class JSON final {
		...
		public:
			...
			std::string ToString() const {
				std::error_code ec;
				if ( std::string ret = ToString(ec); !ec )
					return ret;
				else throw ec;
			}
			...
			std::string ToUnescapedString() const {
				std::error_code ec;
				if ( std::string ret = ToUnescapedString(ec); !ec )
					return ret;
				else throw ec;
			}
	...
	inline JSON JSON::Load(const std::string &str) {
		...
		size_t offset = 0;
		std::error_code ec;
		if ( JSON obj = parsers::parse_next(str, offset, ec); !ec )
			return obj;
		else throw ec;
	}
